...
export function registerClaudeInChromeSkill(): void {
  const allowedTools = getChromeBrowserTools().map((tool) => `mcp__claude-in-chrome__${tool.name}`)

  registerBundledSkill({
    name: 'claude-in-chrome',
    description:
      'Automates your Chrome browser to interact with web pages - clicking elements, filling forms, capturing screenshots, reading console logs, and navigating site. Opens pages in new tabs within your existing Chrome session. Requires site-level permissions before executing (configured in the extension).',
    whenToUse:
