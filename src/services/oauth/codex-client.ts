...
async function postToTokenUrl(body: URLSearchParams): Promise<TokenResult> {
  try {
    ...
    if (!response.ok) {
      ...
      logError(new Error(`[codex-oauth] token endpoint responded ${response.status}: ${text}`))
      return { type: 'failed' }
    }
 ...
 */
export async function exchangeCodexCode(code: string, verifier: string): Promise<CodexTokens> {
