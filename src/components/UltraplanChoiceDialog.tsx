...
import { createUserMessage, createSystemMessage, prepareUserContent } from '../utils/messages.js'
...
export function UltraplanChoiceDialog({
  ...
  const handleChoice = useCallback(
    (choice: UltraplanChoice) => {
      if (choice === 'execute') {
        setMessages((prev) => [
          ...prev,
          createSystemMessage('Ultraplan approved. Executing the following plan:', 'info'),
          createUserMessage({
  ...
  const displayPlan = plan.length > 2000 ? plan.slice(0, 2000) + '\n\n... (truncated)' : plan
        ...
        options={[
          {
            ...
            description: 'Send the plan to Claude for execution in this session',
          },
          {
